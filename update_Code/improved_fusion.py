...
def improved_lidar_camera_fusion(pts_3D, pts_2D, frame, seg_input, obj_class, lidar2cam, erosion_factor=25, depth_factor=20, PCA=False, use_boxes=False):
    """
    改进的LiDAR-相机融合函数
    解决融合失败和3D框过大的问题
    
    Args:
        seg_input: 根据use_boxes参数，可以是：
            - 如果use_boxes=False: 多边形掩码（masks.xy[j]），形状为(N, 2)
            - 如果use_boxes=True: 边界框坐标 [x1, y1, x2, y2]
        use_boxes: 是否使用边界框模式（True=使用box，False=使用mask）
    """
    
    # 2. 根据use_boxes参数选择不同的点云筛选方法
    if use_boxes:
        # 使用边界框模式（快速模式）
        # seg_input 此时是 box 坐标 [x1, y1, x2, y2]
        box = seg_input
        if len(box) != 4:
            print(f"  融合失败: 边界框格式错误 (类别: {obj_class})")
            return None
        
        # 使用腐蚀后的边界框筛选点云（折中方案）
        all_points_of_object = extract_points_in_box_eroded(pts_3D, pts_2D, box, frame, erosion_factor=0.9)
        
        if len(all_points_of_object) == 0:
            print(f"  融合失败: 边界框内无点云 (类别: {obj_class}, box={box})")
            return None
    else:
        # 使用多边形掩码模式（精确模式）
        seg_mask = seg_input
...
def extract_points_in_box_eroded(pts_3D, pts_2D, box, frame, erosion_factor=0.9):
    """
    使用腐蚀后的边界框筛选点云（折中方案：快速且减少背景点）
    
    Args:
        pts_3D: 3D点云数组，形状为(N, 3)
        pts_2D: 2D投影点数组，形状为(N, 2)
        box: 边界框坐标 [x1, y1, x2, y2]
        frame: 图像帧（用于获取尺寸）
        erosion_factor: 腐蚀因子，0.9表示缩小10%，去除边界附近的背景点
    ...
    # 应用腐蚀（缩小边界框以去除边界附近的背景点）
    new_width = width * erosion_factor
    new_height = height * erosion_factor
    ...
    # 直接使用坐标范围筛选（无需创建掩码，性能更高）
    valid_indices = (
        (pts_2D_int[:, 0] >= x1_new) & (pts_2D_int[:, 0] < x2_new) &
        (pts_2D_int[:, 1] >= y1_new) & (pts_2D_int[:, 1] < y2_new)
    )
